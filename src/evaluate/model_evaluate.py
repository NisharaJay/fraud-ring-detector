...
with torch.no_grad():
    ...
    y_pred_all = (out_all > threshold).int() 

# Make DataFrame
results = pd.DataFrame({
    "node": list(G.nodes()),
    "true_label": y.numpy(),
    "pred_label": y_pred_all.numpy()
})

results.to_csv("data/processed/predicted_nodes.csv", index=False)
print("Saved node predictions for all nodes")